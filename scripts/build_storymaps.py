...
- Individual language slides sorted chronologically by earliest recording date
...
def build_title_slide(config: dict[str, Any], slide_count: int,
    ...
    text += (
        f"</p><p>Slides are ordered chronologically by the earliest recording date "
        f"in AILLA's holdings.</p>"
...
def build_summary_slide(below_threshold: pd.DataFrame,
    ...
    if len(restricted) > 0:
        ...
        sections.append(
            ...
            f"significant holdings in AILLA, but a majority of the materials are "
            f"currently restricted or embargoed:</p>"
...
def build_storymap(df: pd.DataFrame, family_name: str,
    ...
    # Languages below threshold entirely
    below = family_df[
        (family_df["total_items"] < threshold) &
        ~(qualifies_public)
    ].copy()
